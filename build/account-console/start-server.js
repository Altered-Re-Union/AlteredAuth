...
// Pin the dev server to the SAME Keycloak version as production (see ../Dockerfile).
// The scaffold defaulted to the "nightly" build, which is newer than the pinned
// @keycloak/keycloak-account-ui and causes REST response-shape mismatches (e.g.
// the ".forEach is not a function" crash when saving with validation errors).
const KEYCLOAK_VERSION = "26.5.0";
...
async function downloadServer(local) {
  ...
  if (local) {
    ...
    console.info(`Downloading and extracting Keycloak ${KEYCLOAK_VERSION}…`);
    const releaseAsset = await getReleaseAsset();
    assetStream = await getAssetAsStream(releaseAsset);
  }
...
async function getAssetAsStream(asset) {
  ...
  if (!response.ok) {
    throw new Error(`Something went wrong requesting the Keycloak ${KEYCLOAK_VERSION} release.`);
  }
